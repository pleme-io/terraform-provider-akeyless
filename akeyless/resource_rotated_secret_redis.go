...
func resourceRotatedSecretRedis() *schema.Resource {
	return &schema.Resource{
		Description: "Redis rotated secret resource",
		Create:      resourceRotatedSecretRedisCreate,
		Read:        resourceRotatedSecretRedisRead,
		Update:      resourceRotatedSecretRedisUpdate,
		Delete:      resourceRotatedSecretRedisDelete,
		Importer: &schema.ResourceImporter{
			State: resourceRotatedSecretRedisImport,
		},
...
func resourceRotatedSecretRedisDelete(d *schema.ResourceData, m interface{}) error {
	return resourceRotatedSecretCommonDelete(d, m)
}

func resourceRotatedSecretRedisImport(d *schema.ResourceData, m interface{}) ([]*schema.ResourceData, error) {

	id := d.Id()

	err := resourceRotatedSecretRedisRead(d, m)
	if err != nil {
		return nil, err
	}
