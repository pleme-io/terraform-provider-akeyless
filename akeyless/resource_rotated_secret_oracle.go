...
func resourceRotatedSecretOracle() *schema.Resource {
	return &schema.Resource{
		Description: "Oracle rotated secret resource",
		Create:      resourceRotatedSecretOracleCreate,
		Read:        resourceRotatedSecretOracleRead,
		Update:      resourceRotatedSecretOracleUpdate,
		Delete:      resourceRotatedSecretOracleDelete,
		Importer: &schema.ResourceImporter{
			State: resourceRotatedSecretOracleImport,
		},
...
func resourceRotatedSecretOracleDelete(d *schema.ResourceData, m interface{}) error {
	return resourceRotatedSecretCommonDelete(d, m)
}

func resourceRotatedSecretOracleImport(d *schema.ResourceData, m interface{}) ([]*schema.ResourceData, error) {

	id := d.Id()

	err := resourceRotatedSecretOracleRead(d, m)
	if err != nil {
		return nil, err
	}
