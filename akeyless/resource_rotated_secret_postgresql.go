...
func resourceRotatedSecretPostgreSql() *schema.Resource {
	return &schema.Resource{
		Description: "Postgresql rotated secret resource",
		Create:      resourceRotatedSecretPostgreSqlCreate,
		Read:        resourceRotatedSecretPostgreSqlRead,
		Update:      resourceRotatedSecretPostgreSqlUpdate,
...
func resourceRotatedSecretPostgreSqlCreate(d *schema.ResourceData, m interface{}) error {
	...
	body := akeyless_api.RotatedSecretCreatePostgresql{
		Name:        name,
	...
	_, _, err := client.RotatedSecretCreatePostgresql(ctx).Body(body).Execute()
	if err != nil {
...
func resourceRotatedSecretPostgreSqlUpdate(d *schema.ResourceData, m interface{}) error {
	...
	body := akeyless_api.RotatedSecretUpdatePostgresql{
		Name:    name,
	...
	_, _, err = client.RotatedSecretUpdatePostgresql(ctx).Body(body).Execute()
	if err != nil {
		if errors.As(err, &apiErr) {
...
func resourceRotatedSecretPostgreSqlDelete(d *schema.ResourceData, m interface{}) error {
	return resourceRotatedSecretCommonDelete(d, m)
}

func resourceRotatedSecretPostgreSqlImport(d *schema.ResourceData, m interface{}) ([]*schema.ResourceData, error) {

	id := d.Id()

	err := resourceRotatedSecretPostgreSqlRead(d, m)
	if err != nil {
		return nil, err
	}
