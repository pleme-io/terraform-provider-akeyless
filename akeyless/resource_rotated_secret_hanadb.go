...
func resourceRotatedSecretHanaDb() *schema.Resource {
	return &schema.Resource{
		Description: "Hanadb rotated secret resource",
		Create:      resourceRotatedSecretHanaDbCreate,
		Read:        resourceRotatedSecretHanaDbRead,
		Update:      resourceRotatedSecretHanaDbUpdate,
		Delete:      resourceRotatedSecretHanaDbDelete,
		Importer: &schema.ResourceImporter{
			State: resourceRotatedSecretHanaDbImport,
		},
...
func resourceRotatedSecretHanaDbDelete(d *schema.ResourceData, m interface{}) error {
	return resourceRotatedSecretCommonDelete(d, m)
}

func resourceRotatedSecretHanaDbImport(d *schema.ResourceData, m interface{}) ([]*schema.ResourceData, error) {

	id := d.Id()

	err := resourceRotatedSecretHanaDbRead(d, m)
	if err != nil {
		return nil, err
	}
