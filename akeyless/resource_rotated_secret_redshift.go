...
func resourceRotatedSecretRedshift() *schema.Resource {
	return &schema.Resource{
		Description: "Redshift rotated secret resource",
		Create:      resourceRotatedSecretRedshiftCreate,
		Read:        resourceRotatedSecretRedshiftRead,
		Update:      resourceRotatedSecretRedshiftUpdate,
		Delete:      resourceRotatedSecretRedshiftDelete,
		Importer: &schema.ResourceImporter{
			State: resourceRotatedSecretRedshiftImport,
		},
...
func resourceRotatedSecretRedshiftCreate(d *schema.ResourceData, m interface{}) error {
	...
	body := akeyless_api.RotatedSecretCreateRedshift{
		Name:        name,
	...
	_, _, err := client.RotatedSecretCreateRedshift(ctx).Body(body).Execute()
	if err != nil {
...
func resourceRotatedSecretRedshiftRead(d *schema.ResourceData, m interface{}) error {
	provider := m.(*providerMeta)
	client := *provider.client
	token := *provider.token
...
func resourceRotatedSecretRedshiftUpdate(d *schema.ResourceData, m interface{}) error {
	...
	body := akeyless_api.RotatedSecretUpdateRedshift{
		Name:    name,
	...
	_, _, err = client.RotatedSecretUpdateRedshift(ctx).Body(body).Execute()
	if err != nil {
		if errors.As(err, &apiErr) {
...
func resourceRotatedSecretRedshiftDelete(d *schema.ResourceData, m interface{}) error {
	return resourceRotatedSecretCommonDelete(d, m)
}

func resourceRotatedSecretRedshiftImport(d *schema.ResourceData, m interface{}) ([]*schema.ResourceData, error) {

	id := d.Id()

	err := resourceRotatedSecretRedshiftRead(d, m)
	if err != nil {
		return nil, err
	}
